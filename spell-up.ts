...
const GuessListView = document.getElementById('guesses') as HTMLDivElement;
const ModalPopup = document.getElementById('modal-popup') as HTMLDivElement;
...
function isWordValid(guess: string): boolean {
    ...
    if (!allWords.includes(guess)) return false;
    if (!areCharsAlpabetical(guess)) return false;
    return guess.localeCompare(latestWord) > 0;
}

function areCharsAlpabetical(word: string): boolean {
    let currentMin = 0;
...
function htmlToElement(html: string): ChildNode | null {
